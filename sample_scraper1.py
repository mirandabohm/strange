...
file = open('wx.csv', 'w', newline= '',encoding='utf-8')
wx_writer = csv.writer(file)

# Begin defining labels that will head .CSV file; adds custom labels later
header_labels = ['Brand','Name']
...
# Set start url
url_base = 'http://__secret__url___[offset]=' 
url = url_base + '0'
...
detail_names = soup.find_all('table', class_= 'product_info_sheet')[0].find('div', class_='cat_spec').find_all('span', class_ = 'cat_overview_spec_name')
for detail_name in detail_names:
    header_labels.append(str(detail_name.text.rstrip(':')))

# Find the total number of products available across all pages
total = soup.find('div', id = 'welcome').find('table', class_='temp').find('p')
...
last_page_offset = find_last_page_offset(total, 10)

# Iterate through, and gather information from, all pages in determined range
for offset in range(0, last_page_offset+10, 20):
...
# =============================================================================
    ...
    all_products = soup.find_all('table', class_= 'spec_prod_sheet')
    ...
    for product in all_products: 
        ...
        product_details = product.find('div', class_='dscrpt_sheet').find_all('span', class_ = 'val_overview')            
        product_details = [product_detail.text for product_detail in product_details]
        
        price_info = product.find('div', class_= 'price')
        
        try:
            price = price_info.find('span', class_= 'reg_prc_val').text
            rebate_available = 'Available'
            rebate_value = price_info.find('span', class_= 'rebate_appl_val').text.strip()
            
        except AttributeError:
            try:
                price = price_info.find('p', class_='over_price_reg_val').text.strip()
        ...
        wx_writer.writerow(row_contents)
